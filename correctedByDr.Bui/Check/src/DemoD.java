	...
	public double evaluate(BufferedWriter bw) throws IOException {
		...
		for (int a = 0; a < 30; a++) {
			...
			if (current[a].contains("<")) {
				...
				/*
				 * for(Bin bins_obj : target){ bw.write(bins_obj.toString() +
				 * " Size= " +bins_obj.currentSize +"\r\n"); }
				 */
				int k = 0;
				int loop = 0;
				...
				/*
				 * int var = Integer.MAX_VALUE; int c = -1; for (int z = 0; z <
				 * temp.size(); z++) { // System.out.println(
				 * temp.get(a).size()); if (var > temp.get(z).size()) { var =
				 * temp.get(z).size(); c = z; } } if (c != -1) objBin =
				 * this.deepCopy(temp.get(c)); bw.write("Bin size " +
				 * objBin.size());
				 */
